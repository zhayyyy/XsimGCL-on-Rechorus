...
class XsimGCLRunner(object):
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument('--num_workers', type=int, default=0, # 5,
                            help='Number of processors when prepare batches in DataLoader')
        parser.add_argument('--pin_memory', type=int, default=True, # 0,
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float: # modified
        model = dataset.model
        ...
        dl = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers,
                        ...
                        pin_memory=True if self.pin_memory != 0 else False, 
        ...
        for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
            ...
            if len(prediction_loss.shape)==2: # only for ranking tasks
                restored_prediction_loss = torch.zeros(*prediction_loss.shape).to(prediction_loss.device)
                # use the random indices to shuffle back
                restored_prediction_loss[torch.arange(item_ids.shape[0]).unsqueeze(-1), indices] = prediction_loss
                out_dict['prediction_loss'] = restored_prediction_loss
